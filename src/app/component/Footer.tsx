...
import { FEATURE_NAMES, getFeatureRouteForRole } from "../lib/featureRoutes";
...
export default function Footer() {
  ...
  const getFeatureHref = (featureName: string) => {
    if (!isAuthenticated) {
      return `/auth/Login?feature=${encodeURIComponent(featureName)}`;
    }
    return getFeatureRouteForRole(featureName, userRole);
  };
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="mt-5 space-y-3">
              {FEATURE_NAMES.map((featureName) => (
                <li key={featureName}>
                  <Link
                    href={getFeatureHref(featureName)}
                    className="group inline-flex items-center gap-2 text-sm text-slate-400 transition-colors duration-200 hover:text-emerald-400"
                  >
                    <FaChevronRight className="h-2.5 w-2.5 text-emerald-500 transition-transform duration-200 group-hover:translate-x-1" />

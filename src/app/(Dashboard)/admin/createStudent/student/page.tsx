...
// import { Shield, BookOpen, Eye, EyeOff, GraduationCap, School } from "lucide-react";
...
//             Join the EduManage platform to get started
//           </p>
//         </div>
...
//                 className={`relative flex flex-col items-center justify-center rounded-xl border p-3.5 text-center transition-all duration-200 focus:outline-none ${
//                   role === "student"
//                     ? "border-[#03204c] bg-blue-50/70 text-[#03204c] ring-2 ring-[#03204c]/20"
...
//                 <span className="text-[10px] text-slate-500">Access courses & tasks</span>
//               </button>
...
//                 <span className="text-[10px] text-slate-500">Manage classes & grades</span>
//               </button>
//             </div>
//           </div>
...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await signUp.email({ name, email, password, role: "student" } as any);
      if (res?.error) {
        setError(res.error.message || "Failed to create account.");
      } else {
    ...
    <main className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-slate-50 px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Join the EduManage platform as a student
          </p>
        </div>

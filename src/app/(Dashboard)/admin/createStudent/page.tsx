...
import React, { useState, ChangeEvent, FormEvent, useMemo, useEffect } from "react";
import { Button, Card, CardHeader, Avatar, AvatarImage, AvatarFallback } from "@heroui/react";
import { ChevronDown, FileSpreadsheet, Upload, Sparkles } from "lucide-react";
import { useRouter } from "next/navigation";
import { useSession } from "@/app/lib/auth-client";
import * as XLSX from "xlsx";
    ...
    <div className="mx-auto w-[90%] px-6 py-10 max-w-7xl">
      ...
      <div className="mb-6 sm:mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <Button
          ...
          onClick={handleAiExcelGenerate}
          isDisabled={generatingAi}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-white border border-slate-200 text-emerald-600 font-semibold hover:bg-emerald-50/50 hover:border-emerald-200 shadow-sm transition text-xs sm:text-sm active:scale-[0.98]"
        >
      ...
      {/* Main Card */}
      <Card className="border border-white/80 bg-white/90 p-6 shadow-xl shadow-slate-900/5 backdrop-blur-xl rounded-3xl sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mb-6 flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:justify-start">
            <Avatar className="h-20 w-20 ring-4 ring-emerald-500/10 shrink-0 rounded-2xl">
              {avatarPreview && <AvatarImage src={avatarPreview} alt="Profile preview" />}
              <AvatarFallback className="bg-emerald-50 text-emerald-600 font-bold text-lg rounded-2xl">
                ST
              </AvatarFallback>
            </Avatar>
            <div className="flex flex-col items-center gap-2 sm:items-start">
              ...
              <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-2xl bg-emerald-50 border border-emerald-100 px-4 py-2.5 text-xs font-bold text-emerald-600 transition-all hover:bg-emerald-100/70 active:scale-[0.98]">
                {uploadingImage ? (
                  <span>Uploading...</span>
                ) : (
                  <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                value={formData.dateOfBirth}
                onChange={handleInputChange}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm text-slate-800 outline-none transition-all duration-200 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/15 [color-scheme:light]"
              />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <select
                  ...
                  value={formData.gender}
                  onChange={handleInputChange}
                  className="w-full appearance-none rounded-2xl border border-slate-200 bg-slate-50/50 pl-4 pr-10 py-3 text-sm text-slate-800 outline-none transition-all duration-200 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/15"
                >
                  <option value="" disabled>Select gender</option>
                  <option value="male">Male</option>
                  <option value="female">Female</option>
            ...
            <div className="mt-6 rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {computedSubjects.map((subject, idx) => (
                  <span
                    key={idx}
                    className="inline-block rounded-xl border border-emerald-200/80 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700"
                  >
                    {subject}
                  </span>
                ))}
          ...
          <div className="mt-8 flex flex-col-reverse justify-end gap-3 pt-4 sm:flex-row sm:items-center">
            <Button
              type="button"
              onClick={handleCancel}
              className="w-full rounded-2xl bg-slate-100 font-bold text-slate-700 hover:bg-slate-200 sm:w-auto px-6 py-3 transition-colors active:scale-[0.98]"
            >
              Cancel
            </Button>
            <Button
              type="submit"
              isDisabled={loading || uploadingImage}
              className="w-full rounded-2xl bg-emerald-500 font-bold text-white shadow-lg shadow-emerald-500/25 hover:bg-emerald-600 sm:w-auto px-6 py-3 inline-flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
            >
              <FileSpreadsheet className="h-4 w-4" />
              {loading ? "Saving & Generating..." : "Create Student & Export Excel"}
            </Button>
